...
(async () => {
  ...
  try {
    for (const vp of [
      ...
      const bookingCount = await page.locator('a[href="https://linktr.ee/fadeoclock"]').count();
      add(`${vp.name}.booking_cta_present`, bookingCount >= 2, String(bookingCount));

      if (vp.name === 'mobile') {
        await page.locator('.menu').click();
        add('mobile.menu_opens', await page.locator('.links').evaluate(el => el.classList.contains('open')));
        await page.locator('.links a[href="#szolgaltatasok"]').click();
        add('mobile.menu_closes_after_link', !(await page.locator('.links').evaluate(el => el.classList.contains('open'))));
      }
    ...
    await browser.close();
    fs.writeFileSync('worker5-ld005022-qa-report.json', JSON.stringify(report, null, 2));
